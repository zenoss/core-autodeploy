...
build=4.2.2-1675
zenoss_base_url="http://artifacts.zenoss.loc/1675/"
...
#    WARNING
#commented for internal url, uncomment for release
...
for service in memcached snmpd; do
	try /sbin/chkconfig $service on
	try /sbin/service $service start
done

#Select Zenpacks
${EDITOR} /opt/zenoss/var/zenpack_actions.txt
...
echo "Configuring and Starting Zenoss"
for service in zenoss; do
        try /sbin/chkconfig $service on
        try /sbin/service $service start
done
